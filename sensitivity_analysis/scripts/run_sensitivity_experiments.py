...
"""
敏感性分析实验运行脚本
在前20个厂上运行敏感性分析实验，结果保存到Google Drive
"""
    ...
    try:
        ...
        if os.path.exists(project_csv):
            ...
            if 'config_file' in df.columns:
                ...
                print(f"📊 项目 {project_id} 已完成敏感性分析实验: {len(completed_configs)} 个")
            else:
                print(f"⚠️ 项目 {project_id} CSV文件缺少config_file列")
        else:
            print(f"📄 项目 {project_id} 敏感性分析结果文件不存在，将从头开始")
    except Exception as e:
        print(f"⚠️ 无法读取项目 {project_id} 敏感性分析结果文件: {e}")
...
def create_project_csv(project_id, drive_path):
    ...
    if not os.path.exists(csv_file):
        ...
        print(f"📄 创建项目敏感性分析CSV文件: {csv_file}")
        return True
    else:
        print(f"📄 项目敏感性分析CSV文件已存在: {csv_file}")
        return True
...
def main():
    ...
    print(f"📊 找到 {len(data_files)} 个数据文件（前20个厂）")
    
    # 获取敏感性分析配置文件
    print("📁 扫描敏感性分析配置文件...")
    config_files = get_sensitivity_config_files()
    print(f"📊 找到 {len(config_files)} 个敏感性分析配置文件")
    
    # 检查是否需要生成配置文件
    if len(config_files) < len(data_files) * 100:
        print("🔧 敏感性分析配置文件不足，正在生成...")
        try:
            ...
            if result.returncode == 0:
                print("✅ 敏感性分析配置文件生成完成")
                config_files = get_sensitivity_config_files()
                print(f"📊 现在有 {len(config_files)} 个敏感性分析配置文件")
            else:
                print(f"❌ 敏感性分析配置文件生成失败: {result.stderr}")
                return
        except Exception as e:
            print(f"❌ 敏感性分析配置文件生成异常: {e}")
            return
    ...
    for project_idx, (project_id, data_file) in enumerate(data_files, 1):
        print(f"\n🚀 开始项目 {project_id} 的敏感性分析实验 ({project_idx}/{len(data_files)})")
        print(f"📁 数据文件: {data_file}")
        
        # 获取该项目的敏感性分析配置文件
        project_configs = [cf for cf in config_files if f"/{project_id}/" in cf]
        print(f"📊 找到 {len(project_configs)} 个敏感性分析配置文件")
        
        if not project_configs:
            print(f"⚠️ 项目 {project_id} 没有敏感性分析配置文件，跳过")
            continue
        
        # 创建项目CSV文件
        if not create_project_csv(project_id, drive_path):
            print(f"❌ 无法为项目 {project_id} 创建敏感性分析CSV文件")
            continue
    ...
    if all_results:
        print(f"💾 总共保存了 {len(all_results)} 个敏感性分析结果")
        print(f"📁 敏感性分析结果保存在: {os.path.join(drive_path, 'SolarPV_Sensitivity_Analysis')}")
